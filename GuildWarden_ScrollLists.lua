...
function libGuildWarden.DefaultShort(TempList,a,b)
	...
	sort(CountList, function(a,b)
		if a[TempList.sort] == nil then a[TempList.sort] = "" end
		if b[TempList.sort] == nil then b[TempList.sort] = "" end
...
function libGuildWarden.ShowUserBox(InfoName)
	...
	if (InfoName) then
		...
		if (AllAbout) then
			...
			frmGuildWardenInfo.CharJoined:SetText("Вступил: ".. Joined)
			frmGuildWardenInfo.CharLeft:SetText("Покинул: ".. Left)
			...
			GuildWardenDeleteChar:SetText("Удалить альта");
			...
			if (Count < 2) then
				GuildWardenDeleteChar:SetText("Недоступно");
